...
it('sorts notes by priority regardless of input order', () => {
  render(
    <NotesSection
      notes={[
        { id: 'n2', week_id: 'w1', project_id: 'p1', priority: 2, note_text: 'Medium prio', author: null },
        { id: 'n0', week_id: 'w1', project_id: 'p1', priority: 0, note_text: 'High prio', author: null }
      ]}
    />
  );
  const high = screen.getByText('High prio');
  const low = screen.getByText('Medium prio');
...
it('renders P3 low priority notes', () => {
  render(<NotesSection notes={[{ id: 'n3', week_id: 'w1', project_id: 'p1', priority: 3, note_text: 'Low priority follow-up', author: null }]} />);

  expect(screen.getByText('P3 · Low')).toBeInTheDocument();
  expect(screen.getByText('Low priority follow-up')).toBeInTheDocument();
});
