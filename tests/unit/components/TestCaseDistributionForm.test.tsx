...
it('submits test case distribution', async () => {
  ...
  fireEvent.change(screen.getByLabelText(/^Automated \/ Automatizados$/i), { target: { value: '20' } });
  fireEvent.change(screen.getByLabelText(/^Pending \/ Pendentes$/i), { target: { value: '5' } });
  fireEvent.change(screen.getByLabelText(/^Not automated \/ Nao automatizados$/i), { target: { value: '3' } });
